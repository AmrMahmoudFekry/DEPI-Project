...
def main():
    ...
    # ── MLflow setup ───────────────────────────────────────
    # NOTE: newer mlflow releases require a database-backed tracking
    # store (not the plain filesystem './mlruns' store) to support
    # Model Registry operations (registered_model_name / stage transitions).
    # A local SQLite file gives us that without needing an external DB server.
    ...
    with mlflow.start_run(run_name=f"FINAL_{best_name}_calibrated"):
        ...
        mlflow.sklearn.log_model(
            ...
            skops_trusted_types=[
                ...
                "utils.helper.SMEFeatureEngineer"
            ]
        )
