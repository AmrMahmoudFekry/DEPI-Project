...
if lang == "ar":
    ...
    .stApp p, .stApp span, .stApp label, .stApp .stMarkdown { text-align: right; }
    .stMarkdown p, .stMarkdown li, .stMarkdown h1, .stMarkdown h2, .stMarkdown h3 { direction: rtl; text-align: right; }
    .stNumberInput input, .stTextInput input, .stSlider, .stSlider * { direction: ltr !important; }
    .stNumberInput label, .stTextInput label, .stSlider label, .stSelectbox label, .stFileUploader label {
        direction: rtl !important; text-align: right !important; display: inline-flex !important;
        align-items: center !important; justify-content: space-between !important; flex-wrap: nowrap !important;
        gap: 0.45rem !important; white-space: nowrap !important;
    }
    .stNumberInput label button, .stTextInput label button, .stSlider label button,
    .stSelectbox label button, .stFileUploader label button { order: -1 !important; margin: 0 !important; padding: 0 !important; align-self: center !important; }
    .stNumberInput label > div, .stTextInput label > div, .stSlider label > div, .stSelectbox label > div, .stFileUploader label > div { order: 0 !important; }
...
def confidence_breakdown(confidence, risk_score):
    ...
    certainty_color = {"High": "#10B981", "Medium": "#F59E0B", "Low": "#EF4444"}[certainty]
    ...
    st.markdown(
        ...
        <div style="background:{'rgba(17,24,39,0.95)' if theme=='dark' else '#F1F5F9'};
            ...
            <div style="font-size:36px;font-weight:800;color:{certainty_color};margin-bottom:6px;">{confidence}%</div>
...
def render_recommendation_card(rec, idx):
    priority_color = {"LOW": "#10B981", "MEDIUM": "#F59E0B", "HIGH": "#EF4444", "CRITICAL": "#DC2626"}.get(rec["priority"], "#2563EB")
    priority_icon = {"LOW": "✅", "MEDIUM": "⚠️", "HIGH": "🔴", "CRITICAL": "🚨"}.get(rec["priority"], "📌")
    ...
    st.markdown(
        f"""
        <div style="background:{box_bg}; border-radius:16px; padding:20px 24px; border:1px solid {box_bdr};
            border-left:4px solid {priority_color}; margin-bottom:12px; direction:{'rtl' if lang=='ar' else 'ltr'};">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
                <div style="font-size:16px;font-weight:700;color:{txt_clr};">{priority_icon} {rec['title']}</div>
                <span style="background:{priority_color}; color:white; padding:4px 12px; border-radius:20px; font-size:11px; font-weight:700;
                    white-space:nowrap; margin-{'right' if lang=='ar' else 'left'}:12px;">
                    {rec['priority']} PRIORITY
                </span>
            </div>
            <div style="color:{sub_clr};font-size:14px;line-height:1.65;">{rec['description']}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
with st.sidebar:
    ...
    if encoded_logo:
        ...
        st.markdown(
            """<div style="text-align:center;margin-top:10px;padding:15px 10px;
                background:linear-gradient(135deg,#2563EB,#06B6D4);border-radius:12px;color:white;font-size:18px;font-weight:800;">
    ...
    cv_auc          = model_summary["model_comparison"][best_model]["cv_roc_auc_mean"] if model_summary and best_model in model_summary["model_comparison"] else "N/A"
    ...
    with foot_c2:
        theme_btn_label = "☀️ Light" if theme == "dark" else "🌙 Dark"
        if st.button(theme_btn_label, use_container_width=True, key="side_theme_toggle"):
            toggle_theme()
...
if page == "Dashboard":
    ...
    st.markdown(f'<div class="main-title">{t("Dashboard Overview")}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="subtitle">{t("AI-powered SME Risk Intelligence Platform")}</div>', unsafe_allow_html=True)
    ...
    if not all(artifact_status.values()):
        st.warning("Some project artifacts are missing. Run `python modeling_pipeline.py` to rebuild the model and comparison files before using predictions and analytics.")
...
# =========================================================
    ...
    st.markdown(f'<div class="main-title">{t("Risk Prediction Engine")}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="subtitle">{t("AI-Powered Financial Risk Assessment")}</div>', unsafe_allow_html=True)

    st.markdown("---")

    # —— Batch Prediction ——————————————————————————————————
    st.markdown(f"## {t('Batch Prediction')}")
    st.markdown(t("Upload a portfolio CSV with one business per row and get a batch risk report. The CSV must contain the same base input features used by the model."))
    ...
    if uploaded_file is not None:
        try:
            ...
            avg_credit_score = round(raw_batch_df['owner_credit_score'].mean(), 0) if 'owner_credit_score' in raw_batch_df.columns else 0
            ...
            with chart_col1:
                ...
                fig_donut = go.Figure(go.Pie(
                    ...
                    marker=dict(colors=donut_colors), textinfo="percent+label",
                    textfont=dict(color=font_color, size=12), pull=[0.03, 0.03, 0.06]
                ))
                fig_donut.update_layout(
                    title="Risk Level Distribution", title_font=dict(color=font_color, size=15),
                    paper_bgcolor=bg_color, font=dict(color=font_color), legend=dict(font=dict(color=font_color)),
            ...
            with chart_col2:
                if 'credit_amount' in batch_results.columns:
                    avg_credit_by_risk = batch_results.groupby('risk_label')['credit_amount'].mean().reindex(['LOW RISK', 'MEDIUM RISK', 'HIGH RISK']).fillna(0).reset_index()
                    fig_bar = go.Figure(go.Bar(
                        x=avg_credit_by_risk['risk_label'], y=avg_credit_by_risk['credit_amount'],
                        marker_color=["#10B981", "#F59E0B", "#EF4444"], text=[f"EGP {v:,.0f}" for v in avg_credit_by_risk['credit_amount']],
                        textposition="outside", textfont=dict(color=font_color, size=11)
                    ))
                    fig_bar.update_layout(
                        title="Avg Credit Amount by Risk Level", title_font=dict(color=font_color, size=15),
                        yaxis=dict(title="Avg Credit (EGP)", tickfont=dict(color=font_color), gridcolor="rgba(100,100,100,0.15)"),
                        xaxis=dict(tickfont=dict(color=font_color)), paper_bgcolor=bg_color, plot_bgcolor=bg_color,
                        font=dict(color=font_color), height=360, margin=dict(l=20, r=20, t=50, b=20),
            ...
            with hist_col:
                fig_hist = go.Figure(go.Histogram(x=batch_results['risk_score'], nbinsx=20, marker_color="#2563EB", opacity=0.85))
                fig_hist.add_vline(x=40, line_dash="dash", line_color="#10B981", annotation_text="Low/Med", annotation_font_color="#10B981")
                fig_hist.add_vline(x=70, line_dash="dash", line_color="#EF4444", annotation_text="Med/High", annotation_font_color="#EF4444")
                fig_hist.update_layout(
                    title="Risk Score Distribution", title_font=dict(color=font_color, size=15),
                    xaxis=dict(title="Risk Score (%)", tickfont=dict(color=font_color)),
                    yaxis=dict(title="Count", tickfont=dict(color=font_color), gridcolor="rgba(100,100,100,0.15)"),
                    paper_bgcolor=bg_color, plot_bgcolor=bg_color, font=dict(color=font_color),
                    height=360, margin=dict(l=20, r=20, t=50, b=20), template="plotly_dark" if theme == "dark" else "plotly_white"
                )
            ...
            with top_col:
                ...
                top5 = batch_results.nlargest(5, 'risk_score')[['risk_score', 'confidence', 'risk_label']].reset_index()
                for _, row in top5.iterrows():
                    st.markdown(
                        f"""<div style="background:rgba(239,68,68,0.1); border-radius:10px;padding:10px 14px; margin-bottom:8px; border-left:3px solid #EF4444;">
                            <div style="color:{font_color};font-size:13px;">
                                <b>Row {int(row['index'])+1}</b> — Risk Score: <b style="color:#EF4444;">{row['risk_score']}%</b> | Conf: {row['confidence']}%
                            </div>
                        </div>""",
                        unsafe_allow_html=True
                    )
            ...
            try:
                ...
                feature_names = pipeline_shap.named_steps["preprocessor"].get_feature_names_out()
 
                classifier = pipeline_shap.named_steps['classifier']
                estimator = classifier.calibrated_classifiers_[0].estimator if hasattr(classifier, 'calibrated_classifiers_') else getattr(classifier, 'base_estimator', classifier)
                ...
                shap_df = pd.DataFrame({'feature': feature_names, 'importance': np.abs(mean_shap), 'direction': np.sign(mean_shap)}).sort_values('importance', ascending=False).head(12)
                shap_df['color'] = shap_df['direction'].map({1.0: '#EF4444', -1.0: '#10B981', 0.0: '#6B7280'})
 
                fig_shap = go.Figure(go.Bar(
                    x=shap_df['importance'][::-1], y=shap_df['feature'][::-1], orientation='h',
                    marker_color=list(shap_df['color'][::-1]), text=[f"{v:.3f}" for v in shap_df['importance'][::-1]],
                    textposition='outside', textfont=dict(color=font_color, size=11)
                ))
                fig_shap.update_layout(
                    title='SHAP Feature Contributions (Batch — Top 12)', title_font=dict(color=font_color, size=15),
                    xaxis=dict(title='Mean |SHAP value|', tickfont=dict(color=font_color)), yaxis=dict(tickfont=dict(color=font_color)),
                    paper_bgcolor=bg_color, plot_bgcolor=bg_color, font=dict(color=font_color), height=440,
                    margin=dict(l=20, r=20, t=50, b=20), template="plotly_dark" if theme == "dark" else "plotly_white"
                )
            ...
            st.download_button(label="📥 Download Full Batch Risk Report (CSV)", data=csv_bytes, file_name="batch_risk_report.csv", mime="text/csv")
    ...
    with col1:
        credit_amount = st.number_input(t("Credit Amount (EGP)"), min_value=0, help=t("The current requested amount or outstanding exposure."), key="inp_credit_amount")
        monthly_income_avg = st.number_input(t("Average Monthly Income (EGP)"), min_value=0, help=t("The business's average monthly revenue / cash inflow."), key="inp_monthly_income")
        total_deposits_3m = st.number_input(t("Total Deposits (3M)"), min_value=0, help=t("Total cash deposited over the last 3 months."), key="inp_total_deposits")

    with col2:
        revenue_volatility_3m = st.slider(t("Revenue Volatility (0-1)"), 0.0, 1.0, 0.3, help=t("Higher values mean more unstable revenue."), key="inp_rev_volatility")
        dti_monthly = st.slider(t("Debt-to-Income Ratio"), 0.0, 1.0, 0.35, help=t("Monthly debt payments divided by monthly income."), key="inp_dti")
        owner_credit_score = st.slider(t("Owner Credit Score"), 300, 850, 650, help=t("Personal credit score of the business owner."), key="inp_credit_score")

    with col3:
        nsf_count_3m = st.number_input(t("NSF Count (3M)"), 0, help=t("Number of non-sufficient-funds events in the last 3 months."), key="inp_nsf_count")
        negative_days_3m = st.number_input(t("Negative Balance Days (3M)"), 0, help=t("Days with a negative balance in the last 3 months."), key="inp_neg_days")
        owner_percentage = st.slider(t("Owner Percentage (%)"), 0.0, 100.0, 50.0, help=t("Equity share controlled by the business owner."), key="inp_owner_pct")

    with st.expander(t("Advanced business details")):
        request_ratio = st.number_input(t("Request Ratio"), min_value=0.0, value=0.1, help=t("Ratio of new funds requested to existing loan commitment."), key="inp_request_ratio")
        business_age_months = st.number_input(t("Business Age (Months)"), min_value=0, value=24, help=t("How long the company has been operating."), key="inp_business_age")
    ...
    if st.button(t("Run AI Analysis 🚀")):
        with st.spinner(t("Running AI Analysis...")):
            form_data = {
                "credit_amount": credit_amount, "business_age_months": business_age_months,
                "monthly_income_avg": monthly_income_avg, "total_deposits_3m": total_deposits_3m,
                "revenue_volatility_3m": revenue_volatility_3m, "request_ratio": request_ratio,
                "dti_monthly": dti_monthly, "nsf_count_3m": nsf_count_3m, "negative_days_3m": negative_days_3m,
                "owner_percentage": owner_percentage, "owner_credit_score": owner_credit_score
            }
            ...
            try:
                ...
                st.session_state.report = {
                    "result": result, "input_df": input_df, "form_data": form_data,
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "recommendations": None
                }
                save_prediction({"risk_score": result["risk_score"], "confidence": result["confidence"], "risk_label": result["risk_label"]})
        ...
        with col1:
            st.plotly_chart(create_gauge_chart(result["risk_score"], theme), use_container_width=True)
        with col2:
            risk_badge(result)
            st.markdown("")
        ...
        with col1:
            st.plotly_chart(create_risk_breakdown_chart(input_df, theme), use_container_width=True)
        with col2:
            st.plotly_chart(create_trend_chart(input_df, theme), use_container_width=True)
        ...
        with st.spinner(t("Generating AI financial advisory...")):
            ai_advisory = generate_ai_recommendations(result, input_df, recommendations, lang=lang)

        box_bg  = "rgba(17,24,39,0.95)" if theme == "dark" else "#FFFFFF"
        box_bdr = "rgba(255,255,255,0.06)" if theme == "dark" else "#CBD5E1"
        st.markdown(f"""<div style="background:{box_bg}; border-radius:16px; padding:24px; border:1px solid {box_bdr}; direction:{'rtl' if lang=='ar' else 'ltr'};">""", unsafe_allow_html=True)
        st.markdown(ai_advisory)
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("---")

        pdf = generate_pdf(result["risk_score"], result["confidence"], result["explanation"], result=result, input_df=input_df, recommendations=recommendations)
        report_filename = f"SME_Risk_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        st.session_state.saved_reports.append({"filename": report_filename, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "risk_label": result["risk_label"], "risk_score": result["risk_score"], "data": pdf})
        st.download_button(label=t("📥 Download Full AI Report (PDF)"), data=pdf, file_name=report_filename, mime="application/pdf")
...
# =========================================================
    ...
    st.markdown(f'<div class="main-title">{t("Analytics Dashboard")}</div>', unsafe_allow_html=True)
    ...
    st.markdown("This dashboard helps decision makers understand SME portfolio risk, identify the strongest risk drivers, and translate model insights into practical credit actions.")
    st.markdown("---")
...
# =========================================================
    ...
    st.markdown(f'<div class="main-title">{t("Reports Center")}</div>', unsafe_allow_html=True)
    ...
    if saved:
        for i, rep in enumerate(reversed(saved)):
            idx = len(saved) - 1 - i
            risk_clr = "#10B981" if "LOW" in rep["risk_label"] else "#F59E0B" if "MEDIUM" in rep["risk_label"] else "#EF4444"
            box_bg  = "rgba(17,24,39,0.95)" if theme == "dark" else "#FFFFFF"
            box_bdr = "rgba(255,255,255,0.06)" if theme == "dark" else "#CBD5E1"
            ...
            with col_info:
                st.markdown(
                    f"""
                    <div style="background:{box_bg}; border-radius:14px; padding:16px 20px; border:1px solid {box_bdr};
                        border-left:4px solid {risk_clr}; margin-bottom:8px;">
                        <div style="font-weight:700;font-size:15px;color:{text};">📊 {rep['filename']}</div>
                        <div style="color:{badge_text};font-size:13px;margin-top:4px;">
                            Generated: {rep['timestamp']} · <span style="color:{risk_clr};font-weight:700;">{rep['risk_label']}</span> · Risk Score: <b>{rep['risk_score']}%</b>
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True
            ...
            with col_btn:
                ...
                st.download_button(label=t("📥 Download"), data=rep["data"], file_name=rep["filename"], mime="application/pdf", key=f"dl_report_{idx}")
    else:
        st.info(t("No PDF reports generated yet. Run an analysis in the **Prediction** page to generate reports."))
    ...
    if not history_df.empty:
        ...
        st.markdown("")
        st.dataframe(history_df.sort_values("timestamp", ascending=False).reset_index(drop=True), use_container_width=True)
        st.markdown("")

        act_col1, act_col2 = st.columns(2)
        with act_col1:
            csv = history_df.to_csv(index=False).encode("utf-8")
            st.download_button(t("📥 Download Prediction History (CSV)"), csv, "prediction_history.csv", "text/csv")
        with act_col2:
