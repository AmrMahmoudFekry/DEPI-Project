...
with st.sidebar:
        ...
        st.markdown(
            f"""<div style="text-align:center;margin-top:10px;">
                <img src="data:image/png;base64,{encoded_logo}"
                    style="width:220px;display:block;margin:auto;object-fit:contain;">
            </div>""",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            """<div style="text-align:center;margin-top:10px;padding:20px 10px;
                background:linear-gradient(135deg,#2563EB,#06B6D4);
                border-radius:16px;color:white;font-size:20px;font-weight:800;">
                📊 SME Risk<br>Intelligence
            </div>""",
            unsafe_allow_html=True
        )
    ...
    st.markdown(f"### {t('Model Snapshot')}")
    st.markdown(
        f"""
        <div style="
            background:{'rgba(17,24,39,0.95)' if theme=='dark' else '#F1F5F9'};
    ...
    st.markdown(f"### {t('AI Status')}")
    st.success(t("Prediction Engine Active"))
    st.info(t("Analytics System Online"))
    st.warning(t("Monitoring Financial Risk"))

    st.markdown("---")

    st.markdown(f"### {t('Platform')}")
    st.caption(t("AI-powered SME Risk Intelligence Platform"))

    st.markdown("<br><br>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown(f"### {t('Theme Mode')}")
